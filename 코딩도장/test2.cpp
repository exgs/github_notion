...
#include <stdlib.h>    // (char *)malloc, free, atof 함수가 선언된 헤더 파일
...
char *getString(JSON *json, const char *key)    // 키에 해당하는 문자열을 가져오는 함수
{
    for (int i = 0; i < TOKEN_COUNT; i++)    // 토큰 개수만큼 반복
    {
...
double getNumber(JSON *json, const char *key)    // 키에 해당하는 숫자를 가져오는 함수
{
    for (int i = 0; i < TOKEN_COUNT; i++)    // 토큰 개수만큼 반복
    {
...
int main()
{
    ...
    char *doc = readFile((const char *)"example.json", &size);
    if (doc == NULL)
        return -1;
    ...
    printf("Title: %s\n", getString(&json, (const char *)"Title"));           // Title의 값 출력
    printf("Year: %d\n", (int)getNumber(&json, (const char *)"Year"));        // Year의 값 출력
    printf("Runtime: %d\n", (int)getNumber(&json, (const char *)"Runtime"));  // Runtime의 값 출력
    printf("Genre: %s\n", getString(&json, (const char *)"Genre"));           // Genre의 값 출력
    printf("Director: %s\n", getString(&json, (const char *)"Director"));     // Director의 값 출력
